...
__STATIC QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
  ...
  int tempC;
//  unsigned char *frameC,*pC;
	FrameFiller *filler;
//	FrameFiller *fillerC;
	int i, j;
	QRcode *qrcode = NULL;
//	QRcode *qrcodeC = NULL;
//  static bool colorFlag=0;
	...
	width = QRspec_getWidth(version);
	frame = QRspec_newFrame(version);
//	frameC = QRspec_newFrame(version);
	if(frame == NULL) {
		QRraw_free(raw);
		return NULL;
	}
	filler = FrameFiller_new(width, frame, 0);
//	fillerC = FrameFiller_new(width, frame, 0);
	if(filler == NULL) {
		QRraw_free(raw);
		free(frame);
		return NULL;
...
//      pC= FrameFiller_next(fillerC);
      ...
      if(colorFlag==1){
        ...
        if(i<raw->dataLength){
          tempC=raw->dataLength-(raw->dataLength %raw->blocks);
          if(i+1 <= tempC){
            *p = (i+1)%raw->blocks;
            if(*p==0){
              *p = raw->blocks;
          ...
          tempC=raw->eccLength-(raw->eccLength %raw->blocks);
          if(i+1-raw->dataLength <= tempC){
            *p = (i+1-raw->dataLength)%raw->blocks;
            if(*p==0){
              *p = raw->blocks;
            }
          }
          else{
            *p = i+1-raw->dataLength-tempC+raw->b1;
          }
        }
//      printf("%d ",*p);
      }
      else if(codeWFlag==1){//各codeword各自編號
        *p =(i+1)%100;
      }
      else if(codeWFlag==2){//各codeword各自編號
        *p =((i+1)-((i+1)%100))/100;
      }
      if(codeWFlag==3){//各codeword依照RS編號
        if(controlFlag==false)RSCount[*p]++;
        controlFlag=true;
        *p =RSCount[*p];
      }
			bit = bit >> 1;
...
//	qrcodeC = QRcode_new(version, width, masked);
EXIT:
	QRraw_free(raw);
	free(filler);
	free(frame);
